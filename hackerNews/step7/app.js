...
function newsList() {
  ...
  getList.push('</ul>');
  getList.push('<div>'); //스타일을 주기위한 <div>
  getList.push(
    `<a href='#/page/${
      store.currentPage > 1 ? store.currentPage - 1 : 1
    }'>이전 페이지</a>`
  );
  getList.push('&nbsp;&nbsp;'); // <a>사이 공백 만들어주기
  getList.push(`<a href='#/page/${store.currentPage + 1}'>다음 페이지</a>`);
  getList.push('</div>');
...
//뉴스 상세 내용 보기
function newsDetail() {
  const id = window.location.hash.substring(7); // id 값에 뭐가 들어가있는지, 상세보기시 앞에 #/show/가 있으니까
...
function router() {
  ...
  if (routePath === '') { /* 처음 요청시에는 해시값이 없으니까 여기가 실행된다. */
    newsList();
  } else if (routePath.indexOf('#/page/') >= 0) {
    /* 해시값 중에서 숫자값만 추출하기, #을 제거하고 문자열 길이를 계산 후 문자열을 숫자로 바꾸어야 한다.*/
